...
def dashboard():
    ...
    branch_counts = {}
    for row in rows:
        branch_counts[row[1]] = branch_counts.get(row[1], 0) + 1

    # Company chart
    company_counts = {}
    for row in rows:
        company = row[2] if row[2] else "Unplaced"
    ...
    avg_package = round(
        sum(r[3] for r in rows if r[3] is not None) / max(1, placed),
        2
    )
